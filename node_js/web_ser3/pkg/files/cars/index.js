...
const removeCar = async (index) => {
    try {
        ...
        const data = await readData(DATA_SOURCE);
        const out = data.filer((_, carIndex) => carIndex !== index);
...
const getCarByIndex = async (index) => {
    try {
        ...
        const data = await readData(DATA_SOURCE);
        // const car = data.filter((c, i) => i === index);
        // josh bolje direkno
        
        if(index > data.length - 1) console.log("Index not in the data");
        return data[index];
    } catch (err) {
        throw err;
    }
